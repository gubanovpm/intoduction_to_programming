...
int main() {
	...
	int k = 0;
	while ((sub[k++] = getc(stdin)) != '\n')						// посимвольное чтение из стандартного потока(из консоли)
		;										// пустой оператор, фактически сюда можно написать k++ и убрать его из выше
												// чтобы получилось 
												// while ((sub[k] = getc(stdin)) != '\n')
												// 	k++;
	int n = strlen(sub);									// n - длина введенной подстроки + 1 на символ перевода каретки
	sub[n - 1] = '\0';									// последним считанным является символ '\n' и его нам надо заменить на '\0' чтобы был соблюден формат строки
	n--;											// так как n изначально было больше на 1 необходимого
