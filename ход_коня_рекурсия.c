...
// мы можем рассматривать игровое поле как неориентированный граф, тогда эта задача - есть задача поиска гамльтонова пути в графе, что является NP полной задачей, а значит нерешаемой за полиномаильное время 
